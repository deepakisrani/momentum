...
describe('blankMeso', () => {
  it('starts with deload every 5, no days', () => {
    expect(blankMeso()).toEqual({ name: '', deloadEveryN: 5, days: [] })
  })
})
...
const full: MesoFull = {
  ...
  days: [
    { id: 'd1', meso_id: 'm1', label: 'Push', order_index: 0, deleted_at: null, exercises: [
...
describe('draftFromFull / stripIds', () => {
  it('maps DB rows to a draft preserving ids', () => {
    ...
    expect(d).toEqual({
      id: 'm1', name: 'June', deloadEveryN: 4,
...
describe('validateMeso', () => {
  const base = () => ({ name: 'X', deloadEveryN: 5, days: [{ label: 'Push', exercises: [{ exerciseId: 'ex1', targetSets: 3, repMin: 8, repMax: 12 }] }] })
  it('passes a valid draft', () => {
    expect(validateMeso(base())).toEqual([])
  })
  it('flags a missing name and zero days', () => {
    expect(validateMeso({ name: ' ', deloadEveryN: 5, days: [] })).toEqual(['name', 'days'])
  })
